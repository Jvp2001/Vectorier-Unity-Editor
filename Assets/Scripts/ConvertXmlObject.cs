...
public class ConvertXmlObject : MonoBehaviour
{
    ...
    public static void ConvertXmlToObject()
    {
        ...
        while (objectFound == false & doc_num < 3) 
        {
            ...
            foreach (XmlNode node in obj.DocumentElement.SelectSingleNode("/Root/Objects"))
            {
                ...
                if (node.Name == "Object")
                    if (node.Attributes.GetNamedItem("Name").Value == FindObjectOfType<ConvertXmlObject>().objectToConvert)
                    {
                        objectFound = true;
                        ...
                        foreach (XmlNode content in node.FirstChild)
                        {
                            if (content.Name == "Image")
                                FindObjectOfType<ConvertXmlObject>().InstantiateObject(content);
        ...
        FindObjectOfType<ConvertXmlObject>().actualObject = null;
    ...
    void InstantiateObject(XmlNode content)
    {
        ...
        if (actualObject == null)
        {
            ...
            actualObject = Instantiate(new GameObject(FindObjectOfType<ConvertXmlObject>().objectToConvert), new Vector3(0, 0, 0), Quaternion.identity);
            DestroyImmediate(GameObject.Find(FindObjectOfType<ConvertXmlObject>().objectToConvert)); //Destroy duplicate
            actualObject.name = FindObjectOfType<ConvertXmlObject>().objectToConvert; //Name it correctly
        }
