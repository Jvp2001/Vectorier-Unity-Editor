...
public class ShowMap : MonoBehaviour
{
    ...
    public static void RenderMap()
    {
        ...
        Debug.Log("Rendering level " + FindObjectOfType<ShowMap>().level_name);
        FindObjectOfType<ShowMap>().lv = new GameObject(FindObjectOfType<ShowMap>().level_name);
        FindObjectOfType<ShowMap>().lv.name = FindObjectOfType<ShowMap>().level_name;

        XmlDocument level = new XmlDocument();
        level.Load(Application.dataPath + "/XML/" + FindObjectOfType<ShowMap>().level_name);
            ...
            if (node.HasChildNodes)
                    ...
                    foreach (XmlNode content in node.FirstChild)
                {
                    if (content.Name == "Object") {
                        FindObjectOfType<ShowMap>().layer+=1;
    ...
    static void RenderSequence(string seq_name, string building_name, string x, string y)
    {
        ...
        foreach (XmlNode node in building.DocumentElement.SelectSingleNode("/Root/Objects"))
        {
            ...
            if (node.Name == "Object")
                ...
                {
                    FindObjectOfType<ShowMap>().part = new GameObject(seq_name);
                    FindObjectOfType<ShowMap>().part.name = seq_name;
                    FindObjectOfType<ShowMap>().part.transform.SetParent(FindObjectOfType<ShowMap>().lv.transform);
                    FindObjectOfType<ShowMap>().part.transform.localPosition = new Vector3(float.Parse(x) / 100, -float.Parse(y) / 100, 0);
                        ...
                        if (content.Name == "Object") {
                            ...
                            if (content.Attributes["Name"] != null) {
                                ...
                                foreach (XmlNode child in content.FirstChild)
                                    {
                                        if (child.Name == "Image")
                                            FindObjectOfType<ShowMap>().InstantiateObject(child, "Unnamed-object", content.Attributes.GetNamedItem("X").Value, content.Attributes.GetNamedItem("Y").Value);
                            ...
                            FindObjectOfType<ShowMap>().actualObject = new GameObject(content.Attributes.GetNamedItem("ClassName").Value);
                            FindObjectOfType<ShowMap>().actualObject.name = content.Attributes.GetNamedItem("ClassName").Value;
                            FindObjectOfType<ShowMap>().actualObject.transform.SetParent(FindObjectOfType<ShowMap>().part.transform);
                            FindObjectOfType<ShowMap>().actualObject.transform.localPosition = new Vector3(0,0,0);
                            FindObjectOfType<ShowMap>().InstantiateObject(content, content.Attributes.GetNamedItem("ClassName").Value, content.Attributes.GetNamedItem("X").Value, content.Attributes.GetNamedItem("Y").Value);
                            FindObjectOfType<ShowMap>().actualObject = null;
                        }
                            ...
                            FindObjectOfType<ShowMap>().actualObject = new GameObject(content.Attributes.GetNamedItem("Name").Value);
                            FindObjectOfType<ShowMap>().actualObject.name = content.Attributes.GetNamedItem("Name").Value;
                            FindObjectOfType<ShowMap>().actualObject.transform.SetParent(FindObjectOfType<ShowMap>().part.transform);
                            FindObjectOfType<ShowMap>().actualObject.transform.localPosition = new Vector3(0,0,0);
                            FindObjectOfType<ShowMap>().InstantiateObject(content, content.Attributes.GetNamedItem("Name").Value, content.Attributes.GetNamedItem("X").Value, content.Attributes.GetNamedItem("Y").Value);
                            FindObjectOfType<ShowMap>().actualObject = null;
                        }
                }

        }
        FindObjectOfType<ShowMap>().part = null;
    }
                    ...
                    if (node.Attributes.GetNamedItem("Name").Value == object_name)
                    {
                        ...
                        foreach (XmlNode content in node.FirstChild)
                        {
                            if (content.Name == "Image")
                                FindObjectOfType<ShowMap>().InstantiateObject(content, object_name, x, y);
